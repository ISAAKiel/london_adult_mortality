...
BoE_ext <- data.frame(BoE$SEQID, BoE$SITE, BoE$SITENAME, BoE$Country, BoE$TIME, BoE$BCENT, BoE$AGE, BoE$Recode_age_cat, BoE$SEX_CAT, BoE$LAT, BoE$LONG, BoE$REGION, BoE$URBAN2)
colnames(BoE_ext) <- c("id", "site_id", "site", "country", "period", "century", "age", "age_code", "sex", "lat", "long", "region", "urban")
BoE_ext <- subset(BoE_ext, age >= 15)
BoE_sites <- BoE_ext %>% group_by(site_id, site, period, lat, long, region, urban) %>% summarize(n = n(), mean_century = median(century))
