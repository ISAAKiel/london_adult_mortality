...
english_wellcome_plot <- ggplotGrob(ggplot(english_wellcome, aes(colour = data, shape = source) ) +  
...
english_wellcome_plot_r <- ggplotGrob(ggplot(english_wellcome_r, aes(colour = data, shape = source) ) +  
  ylab("modal age (corrected for population growth)")  + xlab("year") + ylim(2, 75) +
  geom_errorbar(aes(x = (as.numeric(start) + as.numeric(end)) / 2, y = M, 
                    ymin = HDIlow, ymax=  HDIhigh), width=0, colour = "dark grey") +
  geom_errorbarh(aes(x = (as.numeric(start) + as.numeric(end)) / 2, y = M, 
                     xmax = as.numeric(start), xmin = as.numeric(end), height = 0), colour = "dark grey") +
