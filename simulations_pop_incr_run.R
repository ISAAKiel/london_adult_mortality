if (runCodeNew){
  ...
  for(k in 1:6) {
    ...
    lt_sim_list[[k]] <-   ggplot(lt_sim, aes(y = surv_Gompertz_shape, x = pop_inc_fac) ) + 
      geom_boxplot()  + 
      ylab("Gompertz \u03B2 (MLE)") + ggtitle(paste0("Original Gompertz \u03B2: ", (k + 1)/100) ) +
      xlab("population increase")
    lt_sim_list[[k + 6]] <-   ggplot(lt_sim, aes(y = bayes_gomp_b, x = pop_inc_fac) ) + 
      geom_boxplot()  + 
      ylab("Gompertz \u03B2 (Bayes)") + ggtitle(paste0("Original Gompertz \u03B2: ", (k + 1)/100) ) +
      xlab("population increase")
  }
