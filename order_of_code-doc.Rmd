...
The calculations were made in R using R-Studio. The structure of the code is essentially based on the structure of the text. The pure programme code is in the file order_of_code.R. The file extended with Markdown is order_of_code.RMD and the file order_of_code.pdf generated from it.
...
Note: The base path for rmd files is the folder in which they are located, not the r-project. Consequently, order_of_code.R and order_of_code.RMD are both located in the base folder of the project.

Install required packages, set some options and link the sources for the helper functions. 

Remarck: The current Version of osmplotr has to be installed from github using ```devtools::install_github ("ropensci/osmplotr")```. 
...
English_Peers.R, russell.txt, \newline ```r read.delim("./data/russell.txt", sep = "|", header = F, nrows = 1)[1,1]```
...
Medieval_England.R, Christ_church_monks.txt, \newline ```r read.delim("./data/Christ_church_monks.txt", sep = "|", header = F, nrows = 1)[1,1]```
...
London_1728_1840.R, Mortality_bills_1728_1840.txt, \newline ```r read.delim("./data/Mortality_bills_1728_1840.txt", sep = "|", header = F, nrows = 1)[1,1]```

```{r Basic statistics Mortality bills 1728-1840 age modes, warning=FALSE}
kable(london_1728_1840_ranges, 
      caption = "London Mortality bills 1728-1840, range of age modes M") %>%
  kable_styling(latex_options = "HOLD_position")
```

```{r Basic statistics Mortality bills 1728-1840 rate, warning=FALSE}
kable(london_1728_1840_ranges_r, 
      caption = "London Mortality bills 1728-1840, range of rate") %>%
  kable_styling(latex_options = "HOLD_position")
```

London_1841_raw_all.R, London_1841_raw.txt, \newline ```r read.delim("./data/London_1841_raw.txt", sep = "|", header = F, nrows = 1)[1,1]```
...
English_Mortality.R, wrigley_et_al_1997_england_1640-1809.txt, \newline ```r read.delim("./data/wrigley_et_al_1997_england_1640-1809.txt", sep = "|", header = F, nrows = 1)[1,1]```
...
Data are hard coded in the code. Source? \@Nils: Welche Quelle?

```{r Basic statistics Marylebone, warning=FALSE}
source("./lifetables_processing/Marylebone.R")
kable(Marylebone_ranges, caption = "Marylebone") %>%
  kable_styling(latex_options = "HOLD_position")
```
...
kable(london_1728_1840_result, 
      caption = "London Mortality bills 1728-1840, age modes M") %>%
  kable_styling(latex_options = c("HOLD_position","scale_down"))
kable(london_1728_1840_result_r, 
      caption = "London Mortality bills 1728-1840, rate") %>%
  kable_styling(latex_options = c("HOLD_position","scale_down"))
...
If runCodeNew == TRUE  the file ./lifetables_processing/stbrides_crypt.R will ask for the location of the retrieved dataset (Excel sheet) and process the data. In any other case preprocessed data will be loaded. 
...
The data overview is build during pre-processig in ./chapter_04_results/Wellcome_DB.R and saved to a textfile (sep = \\t).
